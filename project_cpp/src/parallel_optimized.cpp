...
VectorInt predictLocal(const Model& model, const Matrix& inputs) {
    if (inputs.empty()) return VectorInt();
    Matrix a1 = addBiasColumn(inputs);
...
double parallelAccuracy(const Model& model, const Matrix& data, const VectorInt& labels, int root) {
    Matrix localData;
    VectorInt localLabels;
    scatterData(data, labels, localData, localLabels, root);
    
    VectorInt localPred = predictLocal(model, localData);
    
    int localCorrect = 0;
    for (size_t i = 0; i < localPred.size(); i++)
        if (localPred[i] == localLabels[i]) localCorrect++;
    
    int globalCorrect = 0;
    int totalSamples = 0;
    int localSize = localPred.size();
    
    MPI_Reduce(&localCorrect, &globalCorrect, 1, MPI_INT, MPI_SUM, root, MPI_COMM_WORLD);
    MPI_Reduce(&localSize, &totalSamples, 1, MPI_INT, MPI_SUM, root, MPI_COMM_WORLD);
    
    if (rank == root) {
        return static_cast<double>(globalCorrect) / totalSamples;
    }
    return 0.0;
}
...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) {
        std::cout << "========================================\n";
        std::cout << "  MNIST Neural Network - MPI Optimized \n";
        std::cout << "========================================\n";
        std::cout << "Running with " << numProcs << " processes\n";
        std::cout << "Optimization: Parallel Evaluation\n";
    }
    ...
    broadcastMatrix(model.theta1, 0, MPI_COMM_WORLD);
    broadcastMatrix(model.theta2, 0, MPI_COMM_WORLD);
    
    if (rank == 0) std::cout << "\n--- Parallel Evaluation ---\n";
    
    auto evalStart = std::chrono::high_resolution_clock::now();
    
    double trainAcc = parallelAccuracy(model, X_train, y_train, 0);
    double testAcc = parallelAccuracy(model, X_test, y_test, 0);
    
    auto evalEnd = std::chrono::high_resolution_clock::now();
    auto evalDuration = std::chrono::duration_cast<std::chrono::milliseconds>(evalEnd - evalStart);
    double evalTime = evalDuration.count() / 1000.0;
    ...
    if (rank == 0) {
        ...
        std::cout << "Evaluation time: " << std::fixed << std::setprecision(3) << evalTime << "s\n";
